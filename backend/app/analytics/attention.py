...
def generate_workforce_attention_signals(db: Session) -> List[AIInsight]:
    ...
    for dept in departments:
        ...
        if pending_leaves_count >= 2 or (pending_leaves_count / total_dept) >= 0.3:
            ...
            if not existing:
                evidence_text = f"Department '{dept.name}' has {pending_leaves_count} pending leave requests across {total_dept} total headcount."
                insight = AIInsight(
    ...
    for emp in employees:
        ...
        if signal_status in ["Review", "Attention"]:
            ...
            if not existing:
                evidence_text = f"Logged {late_count} late arrivals, {absent_count} absences, and {overtime_days} overtime days in last 20 days."
                insight = AIInsight(
                    employee_id=emp.id,
                    ...
                    explanation=f"Workforce attention signal trigger ({signal_status}) for {emp.first_name} {emp.last_name}.",
                    evidence=evidence_text,
